...
class PlaceOrderService:
    def execute(self, table_token: str, items: list, payment_method: str, added_by: str = 'guest', loyalty_token=None, redeem_loyalty=False) -> TableSession:
        ...
        with transaction.atomic():
            ...
            # Server-authoritative upsell conversion attribution. A claimed rule only
            # counts when it belongs to this restaurant, recommends this exact item,
            # and its trigger is present in the same submitted order.
            trigger_ids = {str(item['menu_item_id']) for item in items}
            claimed_rule_ids = {str(item['upsell_rule_id']) for item in items if item.get('upsell_rule_id')}
            rules = {str(r.id): r for r in UpsellRule.objects.filter(id__in=claimed_rule_ids, restaurant=locked_table.restaurant, is_active=True)}
            if rules:
                from apps.analytics.models import UpsellEvent
                conversion_events = []
                for item in items:
                    rule_id = str(item.get('upsell_rule_id') or '')
                    rule = rules.get(rule_id)
                    if not rule:
                        continue
                    if str(rule.recommended_item_id) != str(item['menu_item_id']) or str(rule.trigger_item_id) not in trigger_ids:
                        continue
                    revenue = db_items[str(item['menu_item_id'])].final_price * item['quantity']
                    conversion_events.append(UpsellEvent(restaurant=locked_table.restaurant, rule=rule, table=locked_table, session=session, event_type='conversion', revenue=revenue))
                UpsellEvent.objects.bulk_create(conversion_events)

            if loyalty_token and added_by == 'guest':
                from apps.loyalty.services import attach_member
                attach_member(session, loyalty_token)
                if redeem_loyalty:
                    from apps.loyalty.services import redeem_reward
                    redeem_reward(session)
