...
export interface RestaurantDiagnostics {
  status: 'ok' | 'attention'
  generated_at: string
  restaurant: { id: string; name: string; slug: string; is_public: boolean }
  dependencies: { database: boolean; cache: boolean; realtime: boolean }
  counts: { tables: number; active_tables: number; menu_items: number; available_menu_items: number; active_staff: number; open_sessions: number }
  integration: { iiko_enabled: boolean; iiko_configured: boolean }
...
export const api = {
  ...
  restaurantDiagnostics(restId: string) {
    return apiRequest<RestaurantDiagnostics>(`/restaurants/${restId}/diagnostics/`)
  },
  ...
  // ── Upsell rules (deterministic, no AI) ──────────────────────────────────

  upsellRules(restId: string) { return apiRequest<UpsellRule[]>(`/restaurants/${restId}/upsell-rules/`) },
  createUpsellRule(restId: string, data: { trigger_item: string; recommended_item: string; priority?: number; is_active?: boolean }) {
    return apiRequest<UpsellRule>(`/restaurants/${restId}/upsell-rules/`, { method: 'POST', body: JSON.stringify(data) })
  },
  updateUpsellRule(restId: string, ruleId: string, data: Partial<{ priority: number; is_active: boolean }>) {
    return apiRequest<UpsellRule>(`/restaurants/${restId}/upsell-rules/${ruleId}/`, { method: 'PATCH', body: JSON.stringify(data) })
  },
  deleteUpsellRule(restId: string, ruleId: string) { return apiRequest<void>(`/restaurants/${restId}/upsell-rules/${ruleId}/`, { method: 'DELETE' }) },

  // ── Central network menu (deterministic, no AI) ───────────────────────────
  networkMenuTemplate(networkId: string) { return apiRequest<NetworkMenuTemplateItem[]>(`/restaurants/networks/${networkId}/menu-template/`) },
  createNetworkMenuTemplateItem(networkId: string, data: Partial<NetworkMenuTemplateItem>) { return apiRequest<NetworkMenuTemplateItem>(`/restaurants/networks/${networkId}/menu-template/`, { method: 'POST', body: JSON.stringify(data) }) },
  deleteNetworkMenuTemplateItem(networkId: string, itemId: string) { return apiRequest<void>(`/restaurants/networks/${networkId}/menu-template/${itemId}/`, { method: 'DELETE' }) },
  publishNetworkMenu(networkId: string, restaurantId?: string) { return apiRequest<{ok:boolean;branches:number;created:number;updated:number}>(`/restaurants/networks/${networkId}/menu-template/publish/`, { method:'POST', body: JSON.stringify(restaurantId ? {restaurant_id:restaurantId} : {}) }) },
  branchMenuOverrides(networkId: string, restaurantId: string) { return apiRequest<NetworkMenuOverride[]>(`/restaurants/networks/${networkId}/branches/${restaurantId}/menu-overrides/`) },
  setBranchMenuOverride(networkId: string, restaurantId: string, itemId: string, data: Partial<NetworkMenuOverride>) { return apiRequest<NetworkMenuOverride>(`/restaurants/networks/${networkId}/branches/${restaurantId}/menu-overrides/${itemId}/`, { method:'PUT', body:JSON.stringify(data) }) },
  ...
  importMenu(restId: string, file: File, dryRun = true) {
    const form = new FormData()
    form.append('file', file)
    form.append('dry_run', dryRun ? 'true' : 'false')
    return apiRequest<MenuImportResult>(`/restaurants/${restId}/menu/import/`, { method: 'POST', body: form })
  },
  ...
  trackUpsell(tableToken: string, ruleId: string, eventType: 'impression' | 'add') {
    return apiRequest<void>(`/guest/${tableToken}/upsell-events/`, { method: 'POST', body: JSON.stringify({ rule_id: ruleId, event_type: eventType }) })
  },
  ...
  placeOrder(data: {
    ...
    items: Array<{ menu_item_id: string; quantity: number; note?: string; upsell_rule_id?: string }>
  }) {
  ...
  loyaltyDashboard(restId: string) {
    return apiRequest<{members_count:number;returning_members:number;repeat_rate:number;visits_count:number;member_spend:number;avg_member_spend:number;redemptions_count:number;discount_given:number;program:{is_enabled:boolean;points_per_1000:number;reward_points:number;reward_discount_amount:number};members:Array<{id:string;visits_count:number;lifetime_spend:number;points:number;last_visit_at:string|null}>}>(`/restaurants/${restId}/loyalty/`)
  },
  updateLoyaltyProgram(restId: string, data: Partial<{is_enabled:boolean;points_per_1000:number;reward_points:number;reward_discount_amount:number}>) {
    return apiRequest(`/restaurants/${restId}/loyalty/program/`, { method: 'PATCH', body: JSON.stringify(data) })
  },
  guestLoyalty(tableToken: string, token: string) {
    return apiRequest<{visits_count:number;lifetime_spend:number;points:number;can_redeem:boolean;points_to_reward:number;program:{is_enabled:boolean;points_per_1000:number;reward_points:number;reward_discount_amount:number}}>(`/guest/${tableToken}/loyalty/${token}/`)
  },
  ...
  // ── Сеть ресторанов / Owner ─────────────────────────────────────────────

  networks() {
    return apiRequest<Array<{ id: string; name: string; owner_restaurant_id: string }>>(`/restaurants/networks/`)
  },

  createNetwork(name: string) {
    return apiRequest<{ id: string; name: string; owner_restaurant_id: string }>(`/restaurants/networks/`, {
      method: 'POST', body: JSON.stringify({ name }),
    })
  },

  networkAnalytics(networkId: string, from?: string, to?: string) {
    const q = new URLSearchParams()
    if (from) q.set('from', from)
    if (to) q.set('to', to)
    const qs = q.toString() ? `?${q}` : ''
    return apiRequest<{
      ...
      totals: { revenue: number; sessions_count: number; avg_check: number; upsell_revenue: number };
      branches: Array<{ id: string; name: string; is_owner_branch: boolean; revenue: number; sessions_count: number; avg_check: number; upsell_revenue: number }>;
    }>(`/restaurants/networks/${networkId}/analytics/${qs}`)
  },
  ...
  createTablesBulk(restId: string, data: { start: number; end: number; zone?: string | null }) {
    return apiRequest<DjangoTable[]>(`/restaurants/${restId}/tables/bulk/`, {
      method: 'POST',
      body: JSON.stringify(data),
    })
  },
  ...
  sessionTimeline(restId: string, sessionId: string) {
    return apiRequest<SessionTimelineResponse>(`/restaurants/${restId}/sessions/${sessionId}/timeline/`)
  },

  serviceRecoveries(restId:string, sessionId:string) { return apiRequest<ServiceRecoveryNote[]>(`/restaurants/${restId}/sessions/${sessionId}/service-recoveries/`) },
  createServiceRecovery(restId:string, sessionId:string, data:{reason:string;note:string;compensation_type?:string;compensation_amount?:number;source_type?:string;source_id?:string}) { return apiRequest<ServiceRecoveryNote>(`/restaurants/${restId}/sessions/${sessionId}/service-recoveries/`, {method:'POST',body:JSON.stringify(data)}) },
  deleteServiceRecovery(restId:string, sessionId:string, recoveryId:string) { return apiRequest<void>(`/restaurants/${restId}/sessions/${sessionId}/service-recoveries/${recoveryId}/`, {method:'DELETE'}) },

  slaIncidents(restId: string) {
    return apiRequest<SLAIncidentResponse>(`/restaurants/${restId}/sla-incidents/`)
  },

  operationsExceptions(restId: string) {
    return apiRequest<OperationsExceptionsResponse>(`/restaurants/${restId}/operations/exceptions/`)
  },
  acknowledgeOperationException(restId: string, exceptionKey: string, action: 'acknowledge'|'unassign'='acknowledge') {
    return apiRequest(`/restaurants/${restId}/operations/exceptions/`, { method:'POST', body:JSON.stringify({ exception_key:exceptionKey, action }) })
  },
...
export interface DjangoRestaurant {
  ...
  kitchen_sla_minutes?: number
  bar_sla_minutes?: number
  ready_to_served_sla_minutes?: number
...
export interface UpsellRule { id: string; trigger_item: string; trigger_item_name?: string; recommended_item: string; recommended_item_name?: string; priority: number; is_active: boolean }

export interface GuestUpsellRule { id: string; trigger_item_id: string; recommended_item_id: string; priority: number }
...
export type MenuImportError = { row: number; field: string; message: string }
export type MenuImportPreviewRow = { row: number; name: string; category: string; price: string; description: string; weight: string; preparation_station: 'kitchen' | 'bar'; is_available: boolean; is_visible: boolean }
export type MenuImportResult = { valid: boolean; total: number; will_create: number; will_skip: number; errors: MenuImportError[]; preview: MenuImportPreviewRow[]; skipped: Array<{row:number; name:string; reason:string}>; created?: number; items?: DjangoMenuItem[] }

export interface NetworkMenuTemplateItem { id:string; name:string; category_name:string; description:string; price:string; image_url:string; emoji:string; weight:string; preparation_station:'kitchen'|'bar'; is_available:boolean; is_visible:boolean; sort_order:number }
export interface NetworkMenuOverride { id:string; template_item:string; restaurant:string; price:string|null; is_available:boolean|null; is_visible:boolean|null; updated_at:string }


export interface SLAAlert { item_id:string; session_id:string; table_number:number; item_name:string; kind:'preparation'|'handoff'; station:string; target_minutes:number; elapsed_minutes:number; severity:'warning'|'overdue' }
export interface SLAIncidentHistory { id:string; item_id:string; session_id:string; table_number:number; item_name:string; kind:'preparation'|'handoff'; station:string; target_minutes:number; breached_at:string; resolved_at:string|null }
export interface SLAIncidentResponse { alerts:SLAAlert[]; history:SLAIncidentHistory[] }
export interface OperationsException { id:string; kind:'sla'|'call'|'payment'|'order'; severity:'critical'|'warning'; title:string; detail:string; created_at:string; action:'session'|'calls'; session_id?:string; call_id?:string; table_number?:number; acknowledged_at?:string|null; acknowledged_by?:{id:string;name:string}|null; response_minutes?:number|null }
export interface OperationsExceptionsResponse { counts:{total:number;critical:number;warning:number;acknowledged:number}; exceptions:OperationsException[]; generated_at:string }

export interface SessionTimelineEvent { at:string; kind:string; title:string; detail:string; severity:'info'|'warning'|'critical'|'success'; item_id?:string|null }
export interface SessionQualitySummary { scope:'full'|'kitchen'; items_total:number; items_completed:number; items_rejected?:number; avg_order_to_confirm_minutes?:number|null; avg_confirm_to_ready_minutes:number|null; avg_ready_to_served_minutes?:number|null; service_duration_minutes?:number; sla_breaches:number; waiter_calls?:number; operational_exceptions?:number; is_closed?:boolean }
export interface SessionTimelineResponse { session_id:string; table_number:number|null; quality_summary:SessionQualitySummary; recovery_sources:Array<{type:'sla'|'exception';id:string;label:string;at:string}>; events:SessionTimelineEvent[] }

export interface ServiceRecoveryNote { id:string; reason:'delay'|'quality'|'wrong_order'|'service'|'payment'|'other'; note:string; compensation_type:string; compensation_amount:string; created_at:string; created_by:string|null; source_type:'sla'|'exception'|null; source_id:string|null }
