...
import {
  Plus, Pencil, Trash2, FolderPlus, ChevronDown, ChevronUp, FileSpreadsheet,
...
import UpsellRulesPanel from './UpsellRulesPanel'
...
export default function MenuManager({
  ...
  return (
    <div className="flex flex-col gap-4 pb-24">
      ...
      <button onClick={() => setImportOpen(true)} className="w-full px-4 py-3 rounded-2xl text-sm font-bold flex items-center justify-center gap-2" style={{ background: 'var(--color-card2)', color: 'var(--color-soft)', border: '1px solid var(--color-rim)' }}><FileSpreadsheet size={16}/> Импорт CSV/XLSX</button>
      </div>
      {importOpen && <MenuImportModal restId={restId} onClose={() => setImportOpen(false)} onImported={load} onToast={onToast} />}

      <UpsellRulesPanel restId={restId} items={items} onToast={onToast} />
