...
export function CartSheet() {
  const { cartOpen, cart, cartNotes, menuItems, set, currentScreen, paymentMethod } = useStore()
  ...
  // Cross-sell: all items NOT already in cart
  const cartIds = new Set(Object.keys(cart).filter(id => (cart[id] ?? 0) > 0))
  const crossSell = menuItems
              ...
              {cartItems.length > 0 && (
                <div className="px-5 pb-8 pt-3 border-t border-rim/50 flex-shrink-0">
                  ...
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <span className="text-gold font-bold text-xl">{cartTotal().toLocaleString('ru')} ₸</span>
                  </div>
                  ...
                  {isTableScreen ? (
                    ...
                    >
                      ...
                      {loading ? t('order_sending') : `${t('checkout_btn')} · ${cartTotal().toLocaleString('ru')} ₸`}
                    </button>
                  ) : (
                    <button
