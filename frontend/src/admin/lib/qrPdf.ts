...
async function renderPage(restaurantName: string, cards: QrPdfCard[]): Promise<Uint8Array> {
  ...
  const margin = 54, gap = 26
  const cellW = (canvas.width - margin * 2 - gap) / 2
  const cellH = (canvas.height - margin * 2 - gap) / 2
  for (let i = 0; i < cards.length; i += 1) {
    const card = cards[i], col = i % 2, row = Math.floor(i / 2)
    const x = margin + col * (cellW + gap), y = margin + row * (cellH + gap)
    ctx.strokeStyle = '#c8c8c8'; ctx.lineWidth = 2; ctx.setLineDash([10, 8]); ctx.strokeRect(x, y, cellW, cellH); ctx.setLineDash([])
    ctx.fillStyle = '#111111'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.font = '700 31px Arial, sans-serif'
    ...
    ctx.font = '800 44px Arial, sans-serif'; ctx.fillText(`Стол №${card.tableNumber}`, x + cellW / 2, y + 128 + qrSize + 70)
    ctx.font = '500 21px Arial, sans-serif'; ctx.fillStyle = '#555555'; ctx.fillText('Сканируйте QR для меню и заказа', x + cellW / 2, y + 128 + qrSize + 118)
    ctx.font = '600 18px Arial, sans-serif'; ctx.fillStyle = '#777777'; ctx.fillText('plait.kz', x + cellW / 2, y + cellH - 44)
  }
...
function buildPdf(pageJpegs: Uint8Array[]): Blob {
  ...
  const catalogId = add(''), pagesId = add(''), pageIds: number[] = []
  for (const jpg of pageJpegs) {
    const imageId = add(concat([bytes(`<< /Type /XObject /Subtype /Image /Width 1240 /Height 1754 /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length ${jpg.length} >>\nstream\n`), jpg, bytes('\nendstream')]))
    const content = bytes('q\n595 0 0 842 0 0 cm\n/Im0 Do\nQ\n')
    const contentId = add(concat([bytes(`<< /Length ${content.length} >>\nstream\n`), content, bytes('endstream')]))
    pageIds.push(add(`<< /Type /Page /Parent ${pagesId} 0 R /MediaBox [0 0 595 842] /Resources << /XObject << /Im0 ${imageId} 0 R >> >> /Contents ${contentId} 0 R >>`))
  }
  ...
  objects.forEach((obj, i) => { offsets.push(cursor); const head = bytes(`${i + 1} 0 obj\n`), tail = bytes('\nendobj\n'); chunks.push(head, obj, tail); cursor += head.length + obj.length + tail.length })
  ...
  const stableBuffer = pdf.buffer.slice(pdf.byteOffset, pdf.byteOffset + pdf.byteLength) as ArrayBuffer
