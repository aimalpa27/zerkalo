...
import { api, type SLAAlert } from '../../lib/api'
...
export default function SessionsTab() {
  const { sessions, staff, tables, zones, profile, isAdmin, djangoRestId, setNewOrderOpen, operationalLoadState, operationalLoadError, operationalLastUpdatedAt, requestOperationalReload } = useAdminStore()
  const [, forceTick] = useState(0)
  const [zoneFilter, setZoneFilter] = useState<string>('all')
  const [slaAlerts, setSlaAlerts] = useState<SLAAlert[]>([])   // 'all' | 'unzoned' | zoneId
  ...
  useEffect(() => {
    ...
    let dead = false
    const load = async () => {
      const id = djangoRestId
      if (!id) return
      try { const d = await api.slaIncidents(id); if (!dead) setSlaAlerts(d.alerts) } catch { /* preserve last snapshot */ }
    }
  ...
  return (
    <div className="px-4 py-4">
      {slaAlerts.length > 0 && <div className="adm-card p-4 mb-4" style={{border:'1px solid rgba(251,113,133,.5)'}}><p className="text-sm font-bold" style={{color:'#fb7185'}}>⚠ SLA: {slaAlerts.filter(a=>a.severity==='overdue').length} просрочено · {slaAlerts.filter(a=>a.severity==='warning').length} скоро SLA</p><p className="text-xs mt-1" style={{color:'var(--color-mid)'}}>{slaAlerts.slice(0,4).map(a=>`Стол ${a.table_number}: ${a.item_name} ${a.elapsed_minutes}/${a.target_minutes}м`).join(' • ')}</p></div>}
