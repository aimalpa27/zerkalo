...
import { Receipt, UtensilsCrossed, Users, BarChart2, LayoutGrid, Bell, Bike, Calendar, MessageCircle, Activity, Building2, HeartHandshake, Siren } from 'lucide-react'
import { useAdminStore } from '../store'
import type { TabId } from '../store'

const ADMIN_TABS = [
  { id: 'exceptions', Icon: Siren, label: 'Проблемы' },
  ...
  { id: 'diagnostics', Icon: Activity,          label: 'Диагностика' },
  { id: 'network', Icon: Building2, label: 'Сеть филиалов' },
  { id: 'loyalty', Icon: HeartHandshake, label: 'Loyalty / CRM' },
] as const
...
export default function AdminSidebar() {
  const { isAdmin, activeTab, calls, chatUnread, setTab, rest, profile } = useAdminStore()
  let tabs: ReadonlyArray<{ id: string; Icon: typeof Receipt; label: string }> = isAdmin
    ? (rest?.features?.delivery ? [...ADMIN_TABS, DELIVERY_TAB] : ADMIN_TABS).filter(t => t.id !== 'network' || profile?.role === 'admin')
    : WAITER_TABS
